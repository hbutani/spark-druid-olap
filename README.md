...
## Use Case: An OLAP index for Star Schemas
Consider the same TPCH Dataset. We also support linking a Druid Index with a **Star Schema**. 
Here is how you would define and query Star Schemas:
...
```
For all the tables see [StarSchemaBaseTest](https://github.com/SparklineData/spark-druid-olap/blob/master/src/test/scala/org/sparklinedata/druid/client/StarSchemaBaseTest.scala)
...
table. We describe how this can be overcome by using aliases/views. But we do support *snowflake* schemas.
...
```
Without the Druid Index the Query will be executed by joining Customer with Orders and than 
...
```
The joins and aggregation in this case are pushed down to the Druid Index.
